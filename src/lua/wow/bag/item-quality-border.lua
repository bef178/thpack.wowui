...
local parseItemLink = ItemUtil.parseItemLink
local getItem = ItemUtil.getItem
...
local function getQualityVertexColor(quality)
...
local function renderQuality(slot, quality)
    if not slot then
        return
    end
...
do
    local function updateAllItemSlots()
        -- carry-on bag slots
        -- TODO
    ...
    hookGlobalFunction("ContainerFrame_OnShow", "post_hook", updateAllItemSlots)
    ...
    f:SetScript("OnEvent", updateAllItemSlots)
end
...
do
    ...
    local function updatePlayerInventorySlots()
